...
export function HelpModal({ opened, onClose }: { opened: boolean; onClose: () => void }) {
  return <Modal opened={opened} onClose={onClose} title="How to Play" size="xl" centered transitionProps={{ duration: 0 }}>
    <Tabs defaultValue="play">
      ...
      <Tabs.Panel value="scoring" pt="md"><Text size="sm"><strong>Score = round(Pips × Mult × XMult).</strong> Every applicable XMult factor multiplies with every other factor; XMult defaults to ×1. Huge factors still compound, but they reward making the qualifying hand.</Text><Text size="sm" mt="sm">Jumping Bean remains an independent score. Hitchhiker can add a held die as a scorer but never helps form the hand shape.</Text></Tabs.Panel>
      ...
      <Tabs.Panel value="enhancements" pt="md">
        <Text size="sm" mb="sm">A physical face holds at most {FACE_TYPE_LIMIT} distinct enhancement types. Extra stacks of an existing type use no new slot. Sticky and Hitchhiker cap at 3 stacks (87.5%). Sustainable has been removed; played hands are consumed normally. During a normal shop, any enhancement type can be scrapped from any face for no refund.</Text>
