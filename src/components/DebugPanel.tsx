    ...
    <Modal opened={opened} onClose={onClose} title="Run Info" size="xl" centered transitionProps={{ duration: 0 }}>
      <Tabs defaultValue="overview">
        ...
        <Tabs.Panel value="overview" pt="md">
          ...
          <Text size="sm" mt="sm"><strong>Gold:</strong> base {state.stats.goldBySource.roundBase} · rerolls {state.stats.goldBySource.unusedRerolls} · interest {state.stats.goldBySource.interest} · Flame Bonus {state.stats.goldBySource.flameBonus} · Golden {state.stats.goldBySource.golden} · Jackpot {state.stats.goldBySource.jackpot}</Text>
          ...
          <Table.ScrollContainer minWidth={760} mt="md"><Table striped><Table.Thead><Table.Tr><Table.Th>Round</Table.Th><Table.Th>Goal</Table.Th><Table.Th>Final</Table.Th><Table.Th>Margin</Table.Th><Table.Th>Base</Table.Th><Table.Th>Rerolls</Table.Th><Table.Th>Interest</Table.Th><Table.Th>Flame Bonus</Table.Th><Table.Th>Total</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>{state.stats.rounds.map(round => <Table.Tr key={round.round}><Table.Td>{round.round}</Table.Td><Table.Td>{round.target}</Table.Td><Table.Td>{round.finalScore}</Table.Td><Table.Td>{round.clearMargin ?? '—'}</Table.Td><Table.Td>{round.payout?.baseGold ?? '—'}</Table.Td><Table.Td>{round.payout?.unusedRerollGold ?? '—'}</Table.Td><Table.Td>{round.payout?.interestGold ?? '—'}</Table.Td><Table.Td>{round.payout?.flameBonusGold ?? '—'}</Table.Td><Table.Td>{round.payout?.totalRoundRewardGold ?? '—'}</Table.Td></Table.Tr>)}</Table.Tbody>
        ...
        <Tabs.Panel value="dice" pt="md"><Table.ScrollContainer minWidth={650}><Table striped><Table.Thead><Table.Tr><Table.Th>Die</Table.Th><Table.Th>Flame</Table.Th><Table.Th>Face</Table.Th><Table.Th>Pips</Table.Th><Table.Th>Enhancements</Table.Th></Table.Tr></Table.Thead>
          <Table.Tbody>{state.dice.flatMap(die => die.faces.map(face => { const id = activeFlameId(die.flame); return <Table.Tr key={`${die.id}:${face.rank}`} className={die.value === face.rank ? 'exposed-face-row' : undefined}><Table.Td>D{die.id + 1}</Table.Td><Table.Td>{id ? `${FLAMES[id].name} ${die.flame!.investedGold}/100` : '—'}</Table.Td><Table.Td>{face.rank}{die.value === face.rank ? ' · exposed' : ''}</Table.Td><Table.Td>{scoringPips(face)}</Table.Td><Table.Td>{ENHANCEMENT_IDS.filter(e => face.enhancements[e]).map(e => `${ENHANCEMENTS[e].name} ×${face.enhancements[e]}`).join(', ') || '—'}</Table.Td></Table.Tr>; }))}</Table.Tbody>
