    ...
    <Modal opened={opened} onClose={onClose} title="Run Info" size="xl" centered transitionProps={{ duration: 0 }}>
      <Tabs defaultValue="overview">
        ...
        <Tabs.Panel value="overview" pt="md">
          ...
          <Table.ScrollContainer minWidth={700} mt="md"><Table striped><Table.Thead><Table.Tr><Table.Th>Round</Table.Th><Table.Th>Goal</Table.Th><Table.Th>Final</Table.Th><Table.Th>Margin</Table.Th><Table.Th>Base</Table.Th><Table.Th>Rerolls</Table.Th><Table.Th>Interest</Table.Th></Table.Tr></Table.Thead>
            <Table.Tbody>{state.stats.rounds.map(round => <Table.Tr key={round.round}><Table.Td>{round.round}</Table.Td><Table.Td>{round.target}</Table.Td><Table.Td>{round.finalScore}</Table.Td><Table.Td>{round.clearMargin ?? '—'}</Table.Td><Table.Td>{round.payout?.base ?? '—'}</Table.Td><Table.Td>{round.payout?.unusedRerolls ?? '—'}</Table.Td><Table.Td>{round.payout?.interest ?? '—'}</Table.Td></Table.Tr>)}</Table.Tbody>
