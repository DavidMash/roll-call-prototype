...
Pips remain literal and Multiplier values may remain fractional. After the final Pips and Multiplier are known, each scoring event awards `Math.round(Pips × Multiplier)` exactly once. Category totals, Effect Score, round score, and target comparisons use those already-rounded integer awards. Sustainable replays round independently before their category awards are accumulated.
...
- Jumping Bean remains independent scoring for initial, manual, post-hand, and chained rolls: its scoring pips × (1 + its own Multiplier stacks × 0.5), rounded once at standalone-score finalization. It does not change a finalized hand accumulator. Its Golden and Workout trigger as usual.

The center playback begins at the hand's Base Pips and Base Multiplier, displays persistent **PIPS / MULT** values as dice and effects build, then shows the whole-number awarded score. Raw fractional calculations and their rounded awards remain available in event-history audit entries. The [scoring audit](docs/scoring-audit.md) records the previous behavior and the domain/event changes.
...
- Export schema **7**, scoring model **rounded-score-accumulator-v4**, includes each hand event's raw mathematical score and rounded awarded score, every final hand level, Hand Training purchases and spend, Sticky/Sustainable probability-proc telemetry, and separate Golden, Jackpot, and round-clear gold income. All rounded hand score belongs to `scoreBySource.hand` and the selected category. The legacy `scoreBySource.hitchhiker` key remains present at zero; schema 1 used it for separate standalone Hitchhiker score. Historical exports retain their original semantics.
